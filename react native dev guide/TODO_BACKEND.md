...
- [] **Registrations API**
  ...
  - Implement `POST /api/v1/registrations` to create a new registration (see **New Registration Flow** below).
  ...
  - **Requirement**: `GET /api/v1/events/{id}` must expose a stable `id` for every `registration_packages` entry.

### New Registration Flow (Native)

The native app follows a "Pay-First, Fill-Later" strategy to minimize friction during the critical checkout phase.

1.  **Quantity Selection**: User selects participant counts for one or more `registration_packages` (e.g., 2x "Early Bird", 1x "Pro").
2.  **Draft Creation**: App calls `POST /api/v1/registrations`. The backend creates a "Draft" registration and automatically generates an "Unpaid" Invoice.
3.  **Payment**: App triggers the Midtrans Snap flow (documented in Phase 4).
4.  **Slot Allocation**: Upon payment success, the backend marks the Registration as "Confirmed" and creates empty "Participant" slots based on the purchased package counts.
5.  **Participant Filling**: The user navigates to the registration detail screen to fill in the attendee details (Name, Category, etc.) for each allocated slot.

### Enhanced Registration API Design

#### `POST /api/v1/registrations`

Supports creating a registration with multiple packages.
**Payload:**
...
```json
{
  "event_id": 123,
  "packages": [
    { "package_id": 1, "count": 2 },
    { "package_id": 2, "count": 1 }
  ]
}
```

#### `GET /api/v1/registrations/{id}` (Registration Resource)

Must include a `package_breakdown` to show the user what they purchased.
**Shape:**
...
```json
{
  "data": {
    ...
    "participants": [
      { "id": 10, "name": "Pending...", "is_filled": false },
      { "id": 11, "name": "John Doe", "is_filled": true }
    ],
...
- [x] **Participants CRUD**
  ...
  - Implement `POST /api/v1/registrations/{id}/participants` to add or update participant details for a specific slot.
